...
# Define the URL for the Matterhorn Bobsleds wait times page
...
def send_notification():
    try:
        ...
        # Iterate through all found ride names to find Matterhorn Bobsleds
        for ride in ride_name_elements:
            ride_name = ride.get_text(strip=True)

            if ride_name == "Matterhorn Bobsleds":
                ...
                if wait_time == "closed":
                    message = "Matterhorn Bobsleds is currently closed."
                else:
                ...
                pushover_response = httpx.post(
                    ...
                    data={
                        "token": api_token,
                        "user": user_key,
                        "title": "Matterhorn Bobsleds Wait Time",
                        "message": message
                    }
                )
                ...
                break  # Exit the loop once we find Matterhorn Bobsleds
...
title = f"Matterhorn Bobsleds: {current_day}"
