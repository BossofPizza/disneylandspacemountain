...
def send_notification():
    try:
        ...
        # Iterate through all found ride names to find Roger Rabbit's Car Toon Spin
        for ride in ride_name_elements:
            ride_name = ride.get_text(strip=True)

            if ride_name == "Roger Rabbit's Car Toon Spin":
                ...
                if lightning_lane_time == "closed":
                    message = "Roger Rabbit's Car Toon Spin's Lightning Lane is currently closed."
                else:
                ...
                pushover_response = httpx.post(
                    ...
                    data={
                        "token": api_token,
                        "user": user_key,
                        "title": "Roger Rabbit's Car Toon Spin Lightning Lane Wait Time",
                        "message": message
                    }
                )
                ...
                break  # Exit the loop once we find Roger Rabbit's Car Toon Spin
