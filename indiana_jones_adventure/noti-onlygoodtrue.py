...
# Define the URL for the Indiana Jones Adventure wait times page
...
def send_notification():
    try:
        ...
        # Iterate through all found ride names to find Indiana Jones Adventure
        for ride in ride_name_elements:
            ride_name = ride.get_text(strip=True)

            if ride_name == "Indiana Jones Adventure":
                ...
                if wait_time == "closed":
                    message = "Indiana Jones Adventure is currently closed."
                else:
                ...
                pushover_response = httpx.post(
                    ...
                    data={
                        "token": api_token,
                        "user": user_key,
                        "title": "Indiana Jones Adventure Wait Time",
                        "message": message
                    }
                )
                ...
                break  # Exit the loop once we find Indiana Jones Adventure
...
while True:
    ...
    # Only send notification if it is a good time to go
    if difference > 10:
